...
function ScrollRevealImageSectionReduced({
...
function ScrollRevealImageSectionAnimated({
  imageSrc = DEFAULT_IMAGE,
  imageAlt = "Architecture and workspace",
  slices = 7,
  heightVh = 280,
...
/**
 * When `prefers-reduced-motion` is on, we must not call `useScroll({ target })` without a mounted ref —
 * Framer Motion throws ("Target ref is defined but not hydrated"). Branch into separate subcomponents.
 */
export default function ScrollRevealImageSection(props: ScrollRevealImageSectionProps) {
  const prefersReducedMotion = useReducedMotion();
  if (prefersReducedMotion === true) {
    return <ScrollRevealImageSectionReduced {...props} />;
  }
  return <ScrollRevealImageSectionAnimated {...props} />;
}
