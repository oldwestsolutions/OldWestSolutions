import Header from '@/components/Header'
import Link from 'next/link'

export default function QuantumComputingPage() {
  return (
    <main className="min-h-screen bg-windows-dark-bg">
      <Header />
      <div className="pt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Link href="/#solutions" className="text-windows-dark-accent hover:text-windows-dark-accent-hover mb-8 inline-block">
            ← Back to Solutions
          </Link>
        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
          <div className="windows-card p-12 md:p-16 bg-gradient-to-br from-windows-dark-accent/30 via-windows-dark-accent/20 to-windows-dark-accent/10 text-center mb-12 md:mb-16">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">Quantum Computing</h1>
            <p className="text-xl md:text-2xl text-windows-dark-text-secondary max-w-3xl mx-auto">
              Explore the next frontier of computing with quantum technologies that solve complex problems beyond classical capabilities.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-16">
            <div className="windows-card p-6 md:p-8">
              <h3 className="text-2xl font-bold text-white mb-4">Quantum Algorithm Development</h3>
              <p className="text-windows-dark-text-secondary mb-4">
                Design and implement quantum algorithms to solve optimization, simulation, and cryptography problems.
              </p>
              <ul className="space-y-2 text-windows-dark-text-secondary">
                <li>• Quantum optimization</li>
                <li>• Quantum simulation</li>
                <li>• Cryptography applications</li>
                <li>• Algorithm design</li>
              </ul>
            </div>

            <div className="windows-card p-6 md:p-8">
              <h3 className="text-2xl font-bold text-white mb-4">Hybrid Quantum-Classical Systems</h3>
              <p className="text-windows-dark-text-secondary mb-4">
                Integrate quantum computing with classical systems for enhanced computational capabilities.
              </p>
              <ul className="space-y-2 text-windows-dark-text-secondary">
                <li>• Hybrid architectures</li>
                <li>• Quantum-classical integration</li>
                <li>• Workflow optimization</li>
                <li>• Performance enhancement</li>
              </ul>
            </div>

            <div className="windows-card p-6 md:p-8">
              <h3 className="text-2xl font-bold text-white mb-4">Quantum Cloud Access</h3>
              <p className="text-windows-dark-text-secondary mb-4">
                Access quantum computing resources through cloud platforms without hardware investment.
              </p>
              <ul className="space-y-2 text-windows-dark-text-secondary">
                <li>• Cloud quantum services</li>
                <li>• Quantum API integration</li>
                <li>• Resource management</li>
                <li>• Cost optimization</li>
              </ul>
            </div>

            <div className="windows-card p-6 md:p-8">
              <h3 className="text-2xl font-bold text-white mb-4">Quantum Machine Learning</h3>
              <p className="text-windows-dark-text-secondary mb-4">
                Leverage quantum computing for machine learning applications with exponential speedups.
              </p>
              <ul className="space-y-2 text-windows-dark-text-secondary">
                <li>• Quantum neural networks</li>
                <li>• Quantum feature maps</li>
                <li>• Quantum optimization</li>
                <li>• Hybrid ML models</li>
              </ul>
            </div>

            <div className="windows-card p-6 md:p-8">
              <h3 className="text-2xl font-bold text-white mb-4">Quantum Security</h3>
              <p className="text-windows-dark-text-secondary mb-4">
                Prepare for post-quantum cryptography and implement quantum-resistant security measures.
              </p>
              <ul className="space-y-2 text-windows-dark-text-secondary">
                <li>• Post-quantum cryptography</li>
                <li>• Quantum key distribution</li>
                <li>• Security assessment</li>
                <li>• Migration planning</li>
              </ul>
            </div>

            <div className="windows-card p-6 md:p-8">
              <h3 className="text-2xl font-bold text-white mb-4">Quantum Consulting</h3>
              <p className="text-windows-dark-text-secondary mb-4">
                Get expert guidance on quantum computing strategy and implementation for your organization.
              </p>
              <ul className="space-y-2 text-windows-dark-text-secondary">
                <li>• Quantum readiness assessment</li>
                <li>• Use case identification</li>
                <li>• Technology evaluation</li>
                <li>• Strategic planning</li>
              </ul>
            </div>
          </div>

          <div className="windows-card p-12 md:p-16 bg-gradient-to-br from-windows-dark-accent/20 to-windows-dark-accent/5 text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">Ready to Explore Quantum Computing?</h2>
            <p className="text-windows-dark-text-secondary mb-10 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
              Discover how quantum computing can solve your most complex challenges and unlock new possibilities.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button className="windows-button windows-button-primary text-lg md:text-xl px-10 py-4">
                Get Started
              </button>
              <button className="windows-button text-lg md:text-xl px-10 py-4">
                Learn More
              </button>
            </div>
          </div>
        </div>
      </div>
    </main>
  )
}

